...
import { fetchSaleLogs } from '../../api/sales';
import { Clock, User as UserIcon, Tag, Info, FileText } from 'lucide-react';
import { jsPDF } from 'jspdf';
import autoTable from 'jspdf-autotable';

const SalesLogsModal = ({ isOpen, onClose, filter, storeId }) => {
    ...
    const loadLogs = async (pageNum, currentFilter = filter) => {
        ...
        try {
            ...
            const data = await fetchSaleLogs(params);
    ...
    const getActionStyle = (action) => {
        ...
        switch (action) {
            case 'SALE': return { ...baseStyle, backgroundColor: 'rgba(34, 197, 94, 0.2)', color: '#4ade80' }; // Green
            case 'EXCHANGE': return { ...baseStyle, backgroundColor: 'rgba(59, 130, 246, 0.2)', color: '#60a5fa' }; // Blue
            case 'VOID': return { ...baseStyle, backgroundColor: 'rgba(239, 68, 68, 0.2)', color: '#f87171' }; // Red
    ...
    const generatePDF = () => {
        try {
            const doc = new jsPDF();
            doc.text("Sales Audit Logs", 14, 15);

            const tableColumn = ["Timestamp", "Action", "Bill No", "Cashier", "Details"];
            const tableRows = [];
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Sales Audit Logs" maxWidth="900px" alignTop={true}>
                        ...
                        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                            <thead>
                                <tr style={{ borderBottom: '1px solid var(--border-color)', textAlign: 'left' }}>
                                    ...
                                    <th style={{ padding: '0.75rem', color: 'var(--text-secondary)' }}>Bill No</th>
                                    <th style={{ padding: '0.75rem', color: 'var(--text-secondary)' }}>Cashier</th>
                            ...
                            <tbody>
                                {logs.map((log) => (
                                    <tr key={log.id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                                        ...
                                        <td style={{ padding: '0.75rem' }}>{log.billNo || log.saleId}</td>
